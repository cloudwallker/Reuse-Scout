...
候选提交 [`27b05139fba0e0e9105b7a66ebc2d1f7dd6e03ae`](https://github.com/cloudwallker/Reuse-Scout/commit/27b05139fba0e0e9105b7a66ebc2d1f7dd6e03ae) 已推送。[第二版 CI](https://github.com/cloudwallker/Reuse-Scout/actions/runs/35602635649) 的四个 job 均实际成功，日志核对结果如下：

| 平台 / Python | 测试总数 / 通过 / 跳过 | 实际耗时 | 跳过范围 |
| --- | --- | --- | --- |
| Windows / 3.9 | 108 / 107 / 1 | 12.825 秒 | 文件名含 `?` 的正例不适用于 Windows |
| Windows / 3.12 | 108 / 107 / 1 | 22.454 秒 | 同上 |
| Ubuntu / 3.9 | 108 / 106 / 2 | 18.999 秒 | Windows 专用联接与短路径用例 |
| Ubuntu / 3.12 | 108 / 106 / 2 | 8.974 秒 | 同上 |

两个 Windows job 的真实 8.3 短路径构建用例及联接拒绝用例均通过；两个 Ubuntu job 的 `%3F` 实际文件名正例均通过。四项的结构校验、构建、工作区及提交空白检查均成功。这是开发工具的跨平台验证，仍不属于原生宿主行为验收。

已从该提交的 Git 对象导出干净源码构建：ZIP **28,620 字节、9 个成员、8 处包内本地引用**，所有成员逐文件匹配源码，CRC、SHA-256 和许可副本一致。摘要：

```text
9f9ea2208e98cf02d03953dae080bf118eb8c45f90638b7a626404c99731f5c0
```

对应 55 个源码文件、提交信息及 ZIP 的再次隐私扫描没有未处置命中；只保留经核对的合成输入和公开 noreply 身份。

随后仅补记本节和收尾记录，形成发布提交，不再改技能、工具或测试；发布仍以该最终提交的四项 CI 成功为条件。最终标签提交、对应 CI 链接、附件摘要及实际下载核验记录写入 [GitHub 预发布说明](https://github.com/cloudwallker/Reuse-Scout/releases/tag/v0.2.0-preview.1)，该链接仅在预发布公开后生效。
